...
// Google News RSS — World agro news (filtered for last 7 days with when:7d)
const WORLD_NEWS_FEEDS = [
    {
        url: 'https://news.google.com/rss/search?q=agricultural+commodities+market+when:7d&hl=uk&gl=UA&ceid=UA:uk',
        label: 'World Markets'
    },
    {
        url: 'https://news.google.com/rss/search?q=wheat+corn+prices+cbot+when:7d&hl=uk&gl=UA&ceid=UA:uk',
        label: 'CBOT Live'
    },
    {
        url: 'https://news.google.com/rss/search?q=agritech+innovation+farming+when:7d&hl=uk&gl=UA&ceid=UA:uk',
...
async function fetchOneFeed(feedObj) {
    try {
        ...
        return items.map(item => {
            ...
            .filter(it => {
                // STRICT FILTER: News must be from the last 7 days
