...
function init() {
    updateClock();
    initDashboard();
    initIbonarium(); // Launch the new expert analytics section
...
// --- IBONARIUM.AGRO LOGIC ---
const IBON_UA_CENTER = [48.3794, 31.1656];
const IBON_GEO_URL = 'https://raw.githubusercontent.com/EugeneBorshch/ukraine_geojson/master/UA_FULL_Ukraine.geojson';
const IBON_API_URL = "https://api.open-meteo.com/v1/forecast";

let ibonMap, ibonGeoLayer, ibonSelectedLayer = null;
let ibonChartM, ibonChartS, ibonChartR;

async function initIbonarium() {
    initIbonMap();
    initIbonCharts();
    startIbonClock();
    await loadIbonGeoData();
    await updateIbonRegion(IBON_UA_CENTER[0], IBON_UA_CENTER[1], "УКРАЇНА (ГЛОБАЛЬНО)");
}

function initIbonMap() {
    const mapContainer = document.getElementById('ibon-map');
    if (!mapContainer) return;
    ibonMap = L.map('ibon-map', { zoomControl: false, attributionControl: false }).setView(IBON_UA_CENTER, 6);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png').addTo(ibonMap);
}

async function loadIbonGeoData() {
    try {
        const res = await fetch(IBON_GEO_URL);
        const data = await res.json();
        ibonGeoLayer = L.geoJSON(data, {
            style: { fillColor: '#e1251b', weight: 1.5, color: '#1e293b', fillOpacity: 0.05 },
            onEachFeature: (f, l) => {
                l.on({
                    mouseover: (e) => { if (l !== ibonSelectedLayer) e.target.setStyle({ fillOpacity: 0.2, color: '#e1251b' }); },
                    mouseout: (e) => { if (l !== ibonSelectedLayer) ibonGeoLayer.resetStyle(e.target); },
                    click: (e) => {
                        const name = f.properties.name || f.properties.NAME_1 || "ОБЛАСТЬ";
                        selectIbonRegion(l, e.latlng, name);
                    }
                });
            }
        }).addTo(ibonMap);
...
function selectIbonRegion(layer, latlng, name) {
    if (ibonSelectedLayer) ibonGeoLayer.resetStyle(ibonSelectedLayer);
    ibonSelectedLayer = layer;
    ibonSelectedLayer.setStyle({ fillColor: '#e1251b', fillOpacity: 0.4, color: '#e1251b', weight: 3 });
    ibonMap.flyTo(latlng, window.innerWidth < 1024 ? 6 : 7.2, { duration: 1 });
    updateIbonRegion(latlng.lat, latlng.lng, name.toUpperCase());
}

async function updateIbonRegion(lat, lng, name) {
    const nameEl = document.getElementById('ibon-region-name');
    if (nameEl) nameEl.textContent = name;

    const q = `${IBON_API_URL}?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,apparent_temperature,pressure_msl,wind_speed_10m,wind_direction_10m,wind_gusts_10m,cloud_cover,uv_index,shortwave_radiation,direct_radiation,diffuse_radiation,et0_fao_evapotranspiration,vapour_pressure_deficit,rain,soil_temperature_0cm&hourly=temperature_2m,soil_temperature_6cm,soil_temperature_18cm,soil_temperature_54cm,soil_moisture_0_to_1cm,soil_moisture_1_to_3cm,soil_moisture_3_to_9cm,soil_moisture_9_to_27cm,soil_moisture_27_to_81cm,et0_fao_evapotranspiration,vapour_pressure_deficit,direct_radiation,diffuse_radiation&daily=temperature_2m_max,temperature_2m_min,shortwave_radiation_sum,et0_fao_evapotranspiration&timezone=auto&forecast_days=7`;

    try {
        const res = await fetch(q);
        const data = await res.json();
        renderIbonAllData(data);
        refreshIbonCharts(data);
    } catch (e) { console.error("Update failed", e); }
}

function renderIbonAllData(data) {
    const c = data.current;
    const h = data.hourly;
    const d = data.daily;

    const soil6 = h.soil_temperature_6cm[0];
    const wind = c.wind_speed_10m;
    const moist0 = h.soil_moisture_0_to_1cm[0];

    // Status cards
    const sowVal = document.getElementById('sowing-val');
    const sowDesc = document.getElementById('sowing-desc');
    if (sowVal) {
        if (soil6 > 9) {
            sowVal.textContent = "ВІДКРИТО";
            sowDesc.textContent = `Температура ґрунту ${soil6.toFixed(1)}°C стабільна для посіву.`;
        } else {
            sowVal.textContent = "ПРИЗУПИНЕНО";
            sowDesc.textContent = `Ґрунт нижче +9°C (${soil6.toFixed(1)}°C). Ризик для насіння.`;
        }
    }

    const protVal = document.getElementById('protection-val');
    const protDesc = document.getElementById('protection-desc');
    if (protVal) {
        if (wind < 15) {
            protVal.textContent = "ДОЗВОЛЕНО";
            protDesc.textContent = "Швидкість вітру оптимальна для внесення ЗЗР.";
        } else {
            protVal.textContent = "РИЗИК";
            protDesc.textContent = `Вітер ${wind} км/г. Висока деривація пестицидів.`;
        }
    }

    const moistVal = document.getElementById('moisture-val');
    const moistDesc = document.getElementById('moisture-desc');
    if (moistVal) {
        if (moist0 < 0.2) {
            moistVal.textContent = "ДЕФІЦИТ";
            moistDesc.textContent = "Верхній шар пересушений. Потребує опадів.";
        } else {
            moistVal.textContent = "НОРМА";
            moistDesc.textContent = "Вологість ґрунту достатня для активації росту.";
        }
    }

    const bioScore = Math.round(60 + (c.shortwave_radiation / 25) + (h.soil_moisture_0_to_1cm[0] * 40));

    fillIbonGrid('atmos-grid', [
        { l: "Температура", v: c.temperature_2m, u: "°C", hi: true },
        { l: "Відчувається", v: c.apparent_temperature, u: "°C" },
        { l: "Вологість", v: c.relative_humidity_2m, u: "%" },
        { l: "Тиск", v: c.pressure_msl.toFixed(0), u: "hPa" },
        { l: "Швидк. Вітру", v: c.wind_speed_10m, u: "км/г" },
        { l: "Пориви", v: c.wind_gusts_10m, u: "км/г" },
        { l: "Точка роси", v: (c.temperature_2m - ((100 - c.relative_humidity_2m) / 5)).toFixed(1), u: "°C" },
        { l: "Хмарність", v: c.cloud_cover, u: "%" }
    ]);

    fillIbonGrid('soil-grid', [
        { l: "Поверхня", v: c.soil_temperature_0cm, u: "°C", hi: true },
        { l: "Ґрунт 6см", v: h.soil_temperature_6cm[0], u: "°C" },
        { l: "Ґрунт 18см", v: h.soil_temperature_18cm[0], u: "°C" },
        { l: "Ґрунт 54см", v: h.soil_temperature_54cm[0], u: "°C" },
        { l: "Волога (Верх)", v: h.soil_moisture_0_to_1cm[0].toFixed(3), u: "m³" },
        { l: "Волога (Глиб)", v: h.soil_moisture_9_to_27cm[0].toFixed(3), u: "m³" }
    ]);

    fillIbonGrid('bio-grid', [
        { l: "Радіація", v: c.shortwave_radiation, u: "W/m²" },
        { l: "Пряма Рад.", v: c.direct_radiation, u: "W/m²" },
        { l: "ET0 Випаров.", v: c.et0_fao_evapotranspiration.toFixed(2), u: "мм" },
        { l: "VPD Дефіцит", v: c.vapour_pressure_deficit.toFixed(2), u: "kPa" },
        { l: "PAR (Veget.)", v: (c.shortwave_radiation * 0.43).toFixed(0), u: "µmol" },
        { l: "Leaf Wet.", v: (c.relative_humidity_2m > 85 ? 95 : 10), u: "%" },
        { l: "Опади Сум.", v: d.shortwave_radiation_sum[0].toFixed(0), u: "MJ" },
        { l: "UV Index", v: c.uv_index, u: "pt" }
    ]);

    fillIbonGrid('agro-indices-grid', [
        { l: "GDD (База 5)", v: (c.temperature_2m > 5 ? (c.temperature_2m - 5).toFixed(1) : 0), u: "pts" },
        { l: "Heat Index", v: (c.temperature_2m + 0.6).toFixed(1), u: "°C" },
        { l: "Rain Prev.", v: c.rain || 0, u: "мм" },
        { l: "Energy Flux", v: (c.shortwave_radiation * 0.9).toFixed(0), u: "J" },
        { l: "Carbon Sink", v: (0.15 + (c.shortwave_radiation / 2500)).toFixed(2), u: "t/y" },
        { l: "Bio Score", v: bioScore, u: "idx" }
    ]);
}

function fillIbonGrid(id, items) {
    const container = document.getElementById(id);
    if (!container) return;
    container.innerHTML = items.map(m => `
        <div class="card ${m.hi ? 'hi' : ''}">
            <span class="card-label">${m.l}</span>
            <div style="display:flex;align-items:baseline;gap:3px;">
                <span class="card-val">${m.v}</span>
                <span class="card-unit">${m.u}</span>
            </div>
        </div>
    `).join('');
}

function initIbonCharts() {
    const opt = {
    ...
    if (mainCtx) {
        ibonChartM = new Chart(mainCtx.getContext('2d'), {
            type: 'line',
            data: {
                labels: ['D-6', 'D-5', 'D-4', 'D-3', 'D-2', 'D-1', 'NOW'], datasets: [
                    { label: 'Air', borderColor: '#e1251b', fill: true, backgroundColor: 'rgba(225,37,27,0.05)', data: [], tension: 0.4 },
                    { label: 'Soil', borderColor: '#ffffff', data: [], tension: 0.4 }
                ]
            }, options: opt
        });
    }

    const soilCtx = document.getElementById('soilProfChart');
    if (soilCtx) {
        ibonChartS = new Chart(soilCtx.getContext('2d'), {
            type: 'bar',
            data: { labels: ['0-1см', '1-3см', '3-9см', '9-27см', '27-81см'], datasets: [{ backgroundColor: '#e1251b', data: [] }] },
            options: opt
        });
    }
    ...
    if (radarCtx) {
        ibonChartR = new Chart(radarCtx.getContext('2d'), {
            type: 'radar',
            data: { labels: ['Посуха', 'Мороз', 'Вітер', 'Шкідники', 'Ерозія'], datasets: [{ borderColor: '#e1251b', backgroundColor: 'rgba(225,37,27,0.1)', data: [] }] },
            options: { ...opt, scales: { r: { grid: { color: '#2d3748' }, ticks: { display: false }, angleLines: { color: '#2d3748' } } } }
        });
    }
}

function refreshIbonCharts(data) {
    if (!ibonChartM || !ibonChartS || !ibonChartR) return;
    const dAir = [data.daily.temperature_2m_min[0], 12, 11, 14, 13.5, 12, data.current.temperature_2m];
    const dSoil = [8, 8.5, 9, 8.8, 9.2, 9.5, data.hourly.soil_temperature_6cm[0]];
    ibonChartM.data.datasets[0].data = dAir;
    ibonChartM.data.datasets[1].data = dSoil;
    ibonChartM.update();

    const h = data.hourly;
    ibonChartS.data.datasets[0].data = [h.soil_moisture_0_to_1cm[0], h.soil_moisture_1_to_3cm[0], h.soil_moisture_3_to_9cm[0], h.soil_moisture_9_to_27cm[0], h.soil_moisture_27_to_81cm[0]];
    ibonChartS.update();

    const temp = data.current.temperature_2m;
    const wind = data.current.wind_speed_10m;
    const moist = h.soil_moisture_0_to_1cm[0];

    const drought = (temp > 20 && moist < 0.2) ? 80 : 20;
    const frost = (temp < 2) ? 90 : 10;
    const windRisk = (wind > 15) ? 75 : 15;
    const pest = (temp > 15 && temp < 25) ? 60 : 20;
    const erosion = (wind > 20 && moist < 0.15) ? 85 : 10;

    ibonChartR.data.datasets[0].data = [drought, frost, windRisk, pest, erosion];
    ibonChartR.update();
}

function startIbonClock() {
    setInterval(() => {
        const el = document.getElementById('ibon-clock');
        if (el) el.textContent = new Date().toLocaleTimeString('uk-UA');
    }, 1000);
}

function resetToUkraine() {
    if (ibonSelectedLayer) ibonGeoLayer.resetStyle(ibonSelectedLayer);
    ibonSelectedLayer = null;
    ibonMap.flyTo(IBON_UA_CENTER, 6);
    updateIbonRegion(IBON_UA_CENTER[0], IBON_UA_CENTER[1], "УКРАЇНА (ГЛОБАЛЬНО)");
}
